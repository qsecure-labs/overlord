...
class main(list):
    ...
    def __init__(self,campaign,mod,project_id):
        ...
        # Call cmd_main class 
        i = cmd_main()
        i.prompt = "(" + cmd2.ansi.style("Overlord", fg=Fg.RED, bg=None,bold=True, underline=False) + " : " + cmd2.ansi.style( project_id, fg=Fg.DARK_GRAY, bg=None,bold=True, underline=False) + cmd2.ansi.style("/c2", fg=Fg.BLUE, bg=None,bold=True, underline=False) +")" +"$> "
        i.cmdloop()
...
class cmd_main(cmd2.Cmd):
    ...
    def do_info(self,mod):
        ...
        if mod :
            ...
            x.add_row(["tools",mod["tools"] , "no", "Tools to be installed on droplet creation."])
            ...
            x.add_row(["tools",self.mod["tools"] , "no", "Tools to be installed on droplet creation."])
            x.align["DESCRITPION"] = "l"
        print(x)
    ...
    parser_redirectors = set_subparsers.add_parser('redirectors', help='Number of redirectors to launch for each c2. Defaults to 1.')
    ...
    parser_tools = set_subparsers.add_parser('tools', help='Tools to be installed on droplet creation.')
    parser_tools.add_argument('tools',nargs="+",choices=tools_list, type=str, help='example: [ set tools < metasploit empire ...>] ')
